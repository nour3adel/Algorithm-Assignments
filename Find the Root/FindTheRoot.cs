...
namespace FindTheRoot
{
    class FindTheRoot
    {
        ...
        public static double bst(int p, int q, int r, int s, int t, int u, double St, double Ed)
        {
            if (St<Ed)
            {
                double mid = (St + Ed) / 2;
                double res = f(p, q, r, s, t, u, mid);
                if (res<eps&&res>-eps)
                {
                    return mid;
                }
                ...
                if (res<0)
                {
                    return bst(p, q, r, s, t, u, St,mid);
                }
                else
                {
                    return bst(p, q, r, s, t, u, mid, Ed);
                }
            }
